...
/* Private variables ---------------------------------------------------------*/
MENU_ITEM_CONTRUCTOR(menu_wycyf, { menu_child_routine(hmenuitem, &(hldr1.Output), "Wyjscia cyfrowe"); } );
...
MENU_ITEM_CONTRUCTOR(menu_btn, { menu_dout_routine(hmenuitem, &(husrbtn.Pull), "BtnS"); } );
...
MenuItem_TypeDef* MENU_MAIN_ARRAY[] = { /* Main menu list */
  ...
  &menu_btn,       /* User Button on-board button state */
...
#define MENU_WYCYF (sizeof(MENU_WYCYF_ARRAY)/sizeof(MENU_WYCYF_ARRAY[0]))
MenuItem_TypeDef* MENU_WYCYF_ARRAY[] = {
		  &menu_ldr1,     /* LED Red #1: on-board red LED (LD3) */
		  &menu_ldg1,     /* LED Green #1: on-board green LED (LD1) */
		  &menu_ldb1,     /* LED Blue #1: on-board blue LED (LD2) */
};
...
void menu_child_routine(MenuItem_TypeDef* hmenuitem, uint32_t channel, const char* name){
